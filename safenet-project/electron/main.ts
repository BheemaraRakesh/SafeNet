...
const createWindow = () => {
  ...
  if (VITE_DEV_SERVER_URL) {
    ...
    // mainWindow.loadFile(path.join(process.env.DIST, 'electron/main.ts'))
  }
}
...
const sendfile = async (fileName: PathLike): Promise<void> => {
  const pythonScriptPath = path.join(__dirname,'prediction.py');
  ...
  try {
    let result = await PythonShell.run(pythonScriptPath, options)
